        ...
        case APP_BT_EVT_RX_DATA:
            if (memcmp(bt_evt->data, "PING", 4) == 0) {
				...
				if(whackamole.game_running) {
					// If the controller is the same as the previously selected one, assume this was a dummy press, and not the fault of the player
					if(player[0].chg_per_index_previous != bt_evt->con_index){
						player[0].score--;
						printk("Wrong button pressed. -1 point. Score %i\n", player[0].score);
					}
				}
            }
				...
				if(bt_evt->con_index == player[0].chg_per_index) {
					if(response_time < whackamole.target_pr_round[whackamole.current_round]) {
						...
						send_color_effect(game, PER_INDEX_ALL, '0', &led_effect_result_good);
					}
					else {
						// Too slow, no points awarded
						printk("Too slow. No points..\n");
						send_color_effect(game, PER_INDEX_ALL, '0', &led_effect_result_bad);
					}
...
static void whackamole_play(struct game_t *game)
{
    ...
    whackamole.round_duration_s = 15;
    whackamole.time = 0;
    whackamole.challenge_int_min = 2;
    whackamole.challenge_int_range = 6;
